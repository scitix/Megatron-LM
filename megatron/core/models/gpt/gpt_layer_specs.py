...
def get_gpt_layer_tree_spec(
    ...
    """GPT layer spec for the native tree-training path.

    Identical to ``get_gpt_layer_with_transformer_engine_spec`` except that
    ``core_attention`` is swapped for ``TETreeDotProductAttention``, which
    consumes ``packed_seq_params.tree_metadata`` and routes through the FA3
    tree kernel. ``cp_size == 1`` is enforced inside the attention class.

    Tree spec does not currently support multi-latent attention or the
    experimental attention variants — those have their own spec providers
    and would need separate tree wrappers if ever required.
    """
    if not HAVE_TE:
        raise ImportError(
            "get_gpt_layer_tree_spec requires Transformer Engine. "
            "Install TE before enabling tree training."
        )

    from megatron.core.extensions.transformer_engine import TETreeDotProductAttention
    ...
    # Replace core_attention in the attention spec with the tree variant.
    # The submodules dataclass is mutated in place because ModuleSpec
    # composes via dataclass field assignment.
    attention.submodules.core_attention = TETreeDotProductAttention
    # tree path expects qkv_format='thd' with no padding mask; make the spec
    # advertise AttnMaskType.arbitrary so the upstream block does not try to
    # synthesize a causal mask that the tree kernel would ignore anyway.
    attention.params["attn_mask_type"] = AttnMaskType.arbitrary
